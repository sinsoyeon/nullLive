...
<script>
	...
	function fn_showWritterDetail(mno){
		...
		console.log(job);
		$.ajax({
			url: "selectContWritterDeatil.jbo",
			type: "get",
			data: {mno:mno,jBtype:jBtype,job:job},
			success: function(data){
				//구인인 경우 매니저 이력을 보여줌
				$("#partnerList").prop("textContent","");
				console.log(job);
				$("#partnerListTitle").prop("textContent","지원자 상세정보 보기(최근 " +job+ "  이력 5건)");
    			if(jBtype == "구인"){
	...
	var job = "${ job }"
		$.ajax({
			...
			success: function(data){
				console.log(data);
				//구인인 경우 매니저 이력을 보여줌
				$("#partnerListTitle").prop("textContent","작성자 상세정보 보기(최근 "+ job +" 이력 5건)");
				$("#partnerList").prop("textContent","");
				$("#partnerList").append("<tr><th>파트너닉네임</th><th>시작일</th><th>종료일</th><th>상태</th></tr>");
				$.each(data , function(i){
					var $tr = $("<tr>");
